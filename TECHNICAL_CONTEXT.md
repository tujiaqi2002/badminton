...
PR #137 合并前通过非破坏性 merge `693ca38` 对齐最新 `main`；[Actions run 32819898640](https://github.com/tujiaqi2002/badminton/actions/runs/32819898640) 在 Node 22 / pnpm 11.16.0 / PostgreSQL 16-alpine 下为 26/26、0 skip。当前 recovery 仍在 `codex/phase-3b-zero-price-activation-recovery`，待 Draft PR/CI；production retry 不在现有授权内。
...
The recovery is on `codex/phase-3b-zero-price-activation-recovery`. Bundled Node `v24.19.0` / pnpm `11.19.0` produced 27 passes, zero failures, and one explicit no-local-PostgreSQL skip across 28 tests; lint/build passed. The added PGlite SQL truth table covers zero-price, positive unpaid/partial/paid, refund, paid-without-ledger, and over-allocation. A recovery PR and pinned Node 22 / pnpm 11.16.0 / PostgreSQL 16 CI are still pending.
