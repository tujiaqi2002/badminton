...
Draft PR #137 实现提交的 [Actions run 32761921315](https://github.com/tujiaqi2002/badminton/actions/runs/32761921315) 已在 Node 22 / pnpm 11.16.0 / PostgreSQL 16-alpine 下完整通过：26/26 tests、0 fail、0 skip，真实 Payment/AA/refund concurrency、lint 和 build 都是 green。Supabase Preview 仍因 Free plan 未配置 preview branch 而预期 skip；它不替代独立 `badminton_stage` hosted apply/matrix/rollback 证据。最新 `main` 整理后的门禁必须重新执行。

2026-08-25 fresh baseline 显示 production `ACTIVE_HEALTHY` / PostgreSQL 17.6 / 44 migrations，3B.1 kernel inactive；staging 为 46 migrations和 activated 17/0/17/3 boundary。Production advisors 是 47 security（2 INFO / 45 WARN）和 62 performance INFO（4 `unindexed_foreign_keys` + 58 `unused_index`）；staging 是 50 security（2 INFO / 48 WARN）和 62 performance INFO（全部 `unused_index`）。旧 staging security 49 少计了新 `admin_link_booking_groups_with_primary` 的 intentional authenticated-definer warning；live catalog 复核确认 `prosecdef=true`、`search_path=\"\"`、函数入口调用 `private.require_manager()`、anon/public 无 EXECUTE，ACL 仅 postgres/authenticated/service_role，因此该 WARN 是预期且受控的 manager entry。
...
独立 `badminton_stage` 已对齐 Draft PR #137 的 46 个版本，最新为 `20260824181500_phase_3b_activation_fk_indexes`。其中 Phase 2 migration 的 DDL/回填逻辑未变，只把四个冻结生产数据指纹替换为合成 fixture 的 staging 指纹。Production 只到 44；activation migrations 45–46 仍不在 `main`，不得在没有新的明确授权时应用。
...
`20260824181500_phase_3b_activation_fk_indexes` adds eight ordered composite-FK indexes. The stage database is aligned to 46 repository versions, all Phase 2/3A/3B diagnostics are clean, the full synthetic writer matrix and hosted contention checks pass, and the performance advisor reports zero unindexed foreign keys. A rollback artifact outside the migration path restores the frozen legacy public writers without deleting append-only history and has been rehearsed inside an outer transaction on the activated stage. Bundled Node `v24.19.0` / pnpm `11.19.0` produced 25 passes, zero failures, and one explicit no-local-PostgreSQL skip across 26 tests; lint and build passed. [Actions run 32761921315](https://github.com/tujiaqi2002/badminton/actions/runs/32761921315) then passed 26/26 tests with zero skips plus lint/build under pinned Node 22 / pnpm 11.16.0 / PostgreSQL 16. All of these gates must be rerun after the latest-main reorganization.

The fresh 2026-08-25 baseline is 44 production migrations and 46 staging migrations. Production has 47 security findings and 62 performance INFO findings, including four unindexed-FK notices. Staging has 50 security findings and 62 performance INFO findings with zero unindexed-FK notices. The former staging security total of 49 undercounted the intentional authenticated-definer warning for the new manager-gated explicit-primary RPC. A live catalog audit confirmed `SECURITY DEFINER`, an empty `search_path`, a leading `private.require_manager()` check, no anonymous/public EXECUTE, and ACL entries only for postgres, authenticated, and service_role; this is therefore an expected controlled manager-entry warning.
