...
| `VITE_RESERVATION_SCHEDULE_READ_SOURCE` | Actions Variable | exact `canonical` 启用馆长排期/容量 canonical read；否则 legacy |
| `VITE_RESERVATION_ORDER_READ_SOURCE` | Actions Variable | exact `canonical` 启用馆长 Reservation order read；否则 legacy |
| `VITE_RESERVATION_SELECTED_DETAIL_READ_SOURCE` | Actions Variable | exact `canonical` 启用馆长 selected-detail canonical read；否则 legacy |
...
合并后的 Pages [run 32838082873](https://github.com/tujiaqi2002/badminton/actions/runs/32838082873) build/deploy 全绿。线上首页返回 200，并引用该 run 生成的 `index-ssrRc0gW.js`；bundle 中 shadow event/flag literals 均为 0。GitHub Actions 未设置 `VITE_RESERVATION_READ_SHADOW`，所以 workflow 使用 `false`，Vite dead-code elimination 已移除 shadow 调用。Production/staging 仍为 48 migrations，合并没有触发 Supabase migration。后续 Phase 4A.3 已完成受控 feature-on observation 并恢复该默认关闭基线；默认 read/UI cutover 与 legacy decommission 仍未授权。

### Phase 4A.3 production shadow observation（Issue #142；已完成并回退）

PR #146 的默认关闭记录于 10:51:36 UTC 合并为 `a205e4a25e2d71d16c13b1e3dc0e4a02b7a31225`；Pages [run 32839332729](https://github.com/tujiaqi2002/badminton/actions/runs/32839332729) 验证线上 `index-ssrRc0gW.js` 不含 shadow event/RPC。用户随后只授权短时生产 shadow observation，明确排除默认读取、任何 writer/mutation、Phase 4B/4C 和 legacy decommission。

临时 exact-true build 由 Pages [run 32839524612](https://github.com/tujiaqi2002/badminton/actions/runs/32839524612) 从相同 main commit 部署为 `index-Dt7sI6uM.js`。已验证的馆长会话加载唯一 URL 并依次触发当前、前一周、返回当前与后一周。浏览器产生 4 个 `info` 级别 clean event；API logs 确认 `admin_list_reservation_allocations` 与 `admin_get_reservation_read_shadow_status` 各 4 次 POST / HTTP 200。根据 logger contract，只有 client allocation comparison 与 normalised server status 同时 clean 时才使用 `info`；mismatch/error 使用 `warn`。

观察后原样运行 `phase_4a_manager_read_contract.sql`，结果仍为 48 migrations、192 bookings/memberships、192 canonical allocations、123 Reservations、17/0/17/3 writer boundary、7 张 FORCE RLS 表、`public.court_slots`-only Realtime，以及所有 Phase 3B/4A mismatch 为 0。没有执行数据库写入或 migration。
...
### Phase 4B.0 local staging frontend/Auth gate（Issue #148；已完成）
...
原 synthetic manager 来自 staging fixture 直接插入的确定性占位 row，没有 password、confirmation 或 `auth.identities`，继续只用于确定性数据库诊断。两个额外的 auto-confirmed disposable password identities 已通过 Supabase Dashboard/Auth Admin 的受支持流程建立；只把真实 synthetic manager 加入 `staff_members(role='admin')`，真实 non-manager 保持无 staff membership。账号凭据未进入仓库、日志或截图。

真实 manager Auth subject 成功调用 canonical schedule/detail RPC，真实 non-manager 得到 `Manager access required`，anon 在函数 ACL 层被拒绝；函数继续是 security invoker、空 `search_path`、authenticated-only。浏览器实际验证中英文 manager schedule/capacity、390×844 手机布局与 non-manager 拒绝页，console error/warn 为 0。最终 hosted diagnostic 仍为 48 migrations、192 bookings/memberships、192 canonical allocations、123 Reservations、Phase 3B/4A mismatch 全 0、17/0/17/3 writer boundary、7 张 FORCE RLS 表与 `public.court_slots`-only Realtime。Production artifact 不含 staging ref，生产 Supabase/Pages/read source 全部未改变。完整边界与中英文证据见 [`docs/reservation-migration/phase-4b0-staging-frontend-gate.md`](./docs/reservation-migration/phase-4b0-staging-frontend-gate.md)。Phase 4B.1 canonical schedule cutover 需新的明确确认。

### Phase 4B.1 canonical schedule/capacity source（Issue #148；production canonical 已启用）

`VITE_RESERVATION_SCHEDULE_READ_SOURCE` 是 build-time fail-closed source selector：只有 exact `canonical` 启用新路径，其他值均为 `legacy`。`.env.example` 明确 legacy，`.env.staging.example` 明确 canonical；PR #151 让生产 workflow 从唯一 repo variable 读取该值，同时缺失值固定回到 legacy。当前 production variable 为 exact `canonical`。`reservationScheduleRead.js` 把 Phase 4A allocation DTO 映射为 explicit version 1 admin schedule view model，保留 physical allocation ID、projection/effective Reservation 与 Session、membership/transition、Party contact、Court/time/amount/status/payment summary，并把 legacy group/link 仅保留为 source trace。任何关键 identity/field 缺失、错误范围或重复 allocation 都 fail closed。

`App.fetchAdminBookings` 在 canonical source 下只调用 `admin_list_reservation_allocations`，复用 venue-timezone boundary 与 `(starts_at, allocation_id)` keyset pagination；range change 通过 AbortController 与 monotonic request ID 防止 stale response。AdminSchedule/AdminCapacity 共用映射后的 rows。错误会清空 rows、保留 bilingual error state 并隐藏 grid，不做 per-request legacy fallback。Legacy shadow comparison 仅在 legacy source 执行。Realtime publication 不变，现有 `court_slots` 事件会按当前 view 刷新馆长 schedule/capacity。

本轮 API 日志从 canonical 验证开始记录 10 次 allocation RPC / HTTP 200，直接 `/rest/v1/bookings` schedule GET 为 0；`admin_search_bookings` 仍服务独立 order area。桌面中英和 390×844 手机均验证 Court 1/Court 2 两条 allocation、剩余 3 courts 与无 overflow。82 tests 为 81 pass / 1 explicit no-local-PostgreSQL skip；lint、production legacy build 与 canonical staging build 通过。Hosted diagnostic 仍为 48 migrations、192/192 memberships/allocations、123 Reservations、0 mismatch、17/0/17/3 writers、7 FORCE RLS、`court_slots`-only Realtime。完整实现记录见 [`docs/reservation-migration/phase-4b1-canonical-schedule.md`](./docs/reservation-migration/phase-4b1-canonical-schedule.md)。

PR [#149](https://github.com/tujiaqi2002/badminton/pull/149) 于 2026-08-25 13:13:04 UTC 合并为 `560ecdc2d738b6bc42d03421e781ba73adac249c`。Pages [run 32852015723](https://github.com/tujiaqi2002/badminton/actions/runs/32852015723) 从同一 commit 完成 build/deploy；线上首页 HTTP 200 并加载 `index-B_GCKKvu.js`。该 bundle 中 `admin_list_reservation_allocations`、staging project ref 与 `VITE_RESERVATION_SCHEDULE_READ_SOURCE` 字面量均为 0，production ref 为 1，证明未设置 selector 时 canonical branch 被编译裁剪。真实线上页面正常渲染且 console error/warn 为 0。上线后只读 Phase 4A diagnostic 仍为 48 migrations、192/192、123、0 mismatch、17/0/17/3、7 FORCE RLS 与 `court_slots`-only Realtime。没有 migration/DB push；生产 canonical default cutover、Phase 4B.2 detail/action 与 decommission 仍是独立门禁。完整发布记录见 [`docs/reservation-migration/phase-4b1-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b1-default-legacy-production-verification.md)。
...
### Phase 4B.2 canonical selected-detail source（Issue #153 / PR #154；production canonical 已启用）
...
`reservationSelectedDetailRead.js` 只调用 `admin_get_reservation_detail(p_reservation_id)`，再把 Phase 4A DTO 映射为 explicit version 1 `AdminReservationInspectorViewModel`。映射白名单覆盖 selection、Reservation、Parties/roles、Sessions/allocations、payment aggregate/shares/safe ledger facts 与 lineage；它要求 selected allocation 在 payload 中恰好一次、属于 selected effective Session，并核对 Court、venue-local range、status、amount/currency、membership version、last transition、summary counts、唯一 primary contact 及 payment references。Payment plan/status 只接受数据库 contract 的显式枚举，未知值 fail closed。

Loader 只在选中 allocation 后请求，按 effective Reservation 在当前 JS 内存去重/缓存；同 Reservation 不同 allocation 会重新执行 identity 映射而不重复 RPC，跨 Reservation 使用 PostgREST `abortSignal` 取消旧请求，并由 React request identity 阻止迟到写回。成功 writer、undo/revert、详情更新和 `court_slots` Realtime 都会失效相关或全部缓存。错误只暴露 allowlisted code，排期继续可见且不会 silent legacy fallback；缓存不进入 storage、console 或数据库。

UI 卡为显式只读，只显示 Reservation/Party/Session/payment/lineage 事实。Edit/payment/link/move/resize/swap/cancel 仍走既有 booking writer contract。数据库、RLS、RPC grants、Realtime 与 Auth 均无变更；完整中英文实现与回退说明见 [`docs/reservation-migration/phase-4b2-canonical-selected-detail.md`](./docs/reservation-migration/phase-4b2-canonical-selected-detail.md)。

PR [#154](https://github.com/tujiaqi2002/badminton/pull/154) 于 2026-08-26 10:19:28 UTC 合并为 `92c892c28bf5458f62f84bc977007b872f3e1014`。Pages [run 32957663853](https://github.com/tujiaqi2002/badminton/actions/runs/32957663853) 成功部署 `index-wknub-uV.js`；production ref 1、staging ref 0。Actions 中 `VITE_RESERVATION_SELECTED_DETAIL_READ_SOURCE` 仍缺失并由 workflow fallback 为 legacy，schedule selector 则继续 canonical。真实馆长选择两条生产 allocation 后 canonical detail DOM 为 0、legacy notes 为 1，API logs 只有成功的 canonical schedule RPC 而没有 `admin_get_reservation_detail`，browser error/warn 为 0。部署前后只读 diagnostic 均保持 48 migrations、192/192、123、0 mismatch、17/0/17/3、7 FORCE RLS 与 `court_slots`-only Realtime。完整证据见 [`docs/reservation-migration/phase-4b2-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b2-default-legacy-production-verification.md)。
...
Migration preflight 要求精确 48-version Phase 4A baseline、clean Phase 3B activation/read assertions、既有 security shape 与 grants；postflight 再验证定义、ACL 和 predecessor contracts。RPC 继续是 `STABLE`、`SECURITY INVOKER`、`search_path = ''`，函数内独立检查 `auth.uid()` 与 `staff_members.role = 'admin'`，并只 grant authenticated EXECUTE。Anon/service_role 无入口；没有业务数据 mutation、RLS/client DML、Realtime、Auth、pricing 或 payment-fact 变化。
...
Hosted stage dry-run 只列 migration 49，`db push` 后 staging 精确为 49，而 production 保持 48。Stage 基线为 123 Reservations / 135 Sessions / 192 memberships / 131 Parties；manager annual query 首屏 50、summary 83 / 9,120 minutes，secondary Party search 返回一笔 `party_count = 2` Reservation。Authenticated non-manager 被拒绝，ACL 为 authenticated only。浏览器把 future-30 从 10 legacy rows 收敛为 5 Reservation cards，并验证两 allocation focus、中英 desktop、390×844 mobile 与零 console error/warn。完整中英文证据见 [`docs/reservation-migration/phase-4b3-canonical-order-search.md`](./docs/reservation-migration/phase-4b3-canonical-order-search.md)。

Merge 前的 fresh production preflight 已只读完成：migration list 为 remote-applied 48、remote-only 0、local-only 仅 `20260826181644`；`db push --dry-run` 同样只列 migration 49。Phase 3B/4A diagnostic、FORCE RLS、writer 边界和 `court_slots`-only Realtime 均保持 clean；non-manager role test 在 read-only transaction 内按预期返回 `Manager access required`。Security advisor 的 48 项与 performance advisor 的 60 项均为既有 baseline，没有 Phase 4B.3-specific finding。该门禁结束时 PR #158 仍为 Draft，尚无 merge、production push 或数据 mutation；随后才取得独立 merge/自动 migration 49 授权。
...
### Phase 4C.1 canonical profile mutation（Issue #161 / #162 / PR #163；staging / production 均为 51，production UI legacy）
...
Hosted PostgreSQL 17.6 rollback tests 覆盖三个 scope、stale、invalid、non-manager、idempotency 和真实 merge 后的反向 Party lineage；最终业务与 operation/audit 测试增量为 0，Phase 3B/4A、incomplete operation 与 Realtime diagnostic clean。Advisor 相对 stage baseline 只新增一条有意的 authenticated-callable security-definer warning，performance 无新增。

用户在独立 Ready/merge gate 后授权，PR #163 合并为 `a387a0a844084eb52a905db2fe92e161c231253b`，protected integration 将 production 从 49 原子推进到 51。Postflight 验证 1 public / 5 private function inventory、authenticated-only ACL、双向 Party lineage、0 operation/audit、Phase 3B/4A clean 与 `court_slots`-only Realtime。Security advisor 从 48 增至 49，唯一新增项是目标 RPC 的预期 lint 0029；performance 保持 60 INFO。
...
四张 private Phase 3 internal tables 未启用 RLS，但 `private` schema 对 anon/authenticated 无 USAGE，且四表 client DML/SELECT grants 均为 0；因此不根据通用 inventory warning 自动加 policy。Current official stage advisors 为 51 security（既有 2 INFO / 49 WARN）与 56 performance unused-index INFO，没有 R2 persistent-object finding。完整 evidence 见 [`docs/reservation-migration/reservation-reset-r2-stage-rehearsal.md`](./docs/reservation-migration/reservation-reset-r2-stage-rehearsal.md)。R2 文件禁止改 ref 后复用；后续 R3A 已另行刷新 production artifacts，destructive execution 仍是独立门禁。

### Reservation reset R3A（Issue #165；production preflight/recovery complete，execution disabled）

R3A 在 production 只执行显式 read-only transactions。Fresh manifest 仍为 PostgreSQL 17.6 / 51 migrations，冻结 15 preserve selectors / 206 rows / SHA-256 `d5c5186d647d6f5a9d8f552d886e92773733905821694be1d28b381ac045310f`，以及 25 database purge selectors / 4,327 rows / SHA-256 `c945049e3725602fd00a9e963591962e74744f96bf89852d32143f384a8cb39c`。Audit 使用 booking/event/member exact entity-ID set，purge 1,661、preserve 77；所有 transaction facts 仍为 test-only legacy data，ledger CAD 1,642.00，无 provider reference。

Fresh backup writer 只接受 repo-external `header/chunk/footer` ciphertext JSONL，并通过 exclusive create、0600 mode 与 plaintext-field rejection 防止误落盘；Windows 目录 ACL 关闭 inheritance，只保留 operator/SYSTEM。Production 内 AES-256 加密后形成 66 relations / 4,687 rows / 242 chunks / 1,106,060-byte artifact，SHA-256 `7e4e3f877940cc92e79268ae28f71211097e71efaa12bdb9775b256fd377f115`；DPAPI CurrentUser key 为独立文件。Production 66/66 fingerprints before/after 相同。Verifier 直接从解密后的 PostgreSQL JSONB 文本比较 bigint/FK，避免 JavaScript number 精度，并在 PGlite 18.3 内存中通过 66 fingerprints、manager/Auth graph、Reservation references、ledger 和 migrations；plaintext persisted=false。

Production draft [`reservation_reset_r3b_production_draft.sql`](./supabase/maintenance/reservation_reset_r3b_production_draft.sql) 不在 migration path，固定 `tiger.r3b.execution_authorized=false`。它使用 serializable transaction、5-second lock timeout、try-advisory lock、单一固定 table-lock order、40 个 selector count/fingerprint gates、28 个 Auth FK catalog/count gates、explicit child-to-parent deletes、transaction-local `session_replication_role`、preserve/RLS/Realtime/migration/sequence postflight 和 negative-ID audit marker；不使用 TRUNCATE、不重置 sequence、不删除 `auth.*`。唯一 non-manager Auth candidate 通过 admin staff complement + UUID SHA-256 选择，repo 不存 UUID/email；当前 Storage owner/owner_id 均为 0。

Database commit 与 Auth deletion 是不可合并的恢复边界：DB marker 先返回 Auth pending，随后才可按 [`reservation-reset-r3b-auth-runbook.md`](./docs/reservation-migration/reservation-reset-r3b-auth-runbook.md) 使用 Dashboard/Admin API 删除 candidate graph。Access JWT 在自身 expiry 前仍可能通过签名验证，session/refresh 清理和实际 JWT expiry 必须单独观察。R3A 没有 production/Auth write，hosted production restore RTO 未测量；future R3B 必须 fresh recheck、final runner hash review 和再次明确 destructive authorization。完整 evidence 见 [`reservation-reset-r3a-production-preflight.md`](./docs/reservation-migration/reservation-reset-r3a-production-preflight.md)。
...
- 生产最新：`20260827090512_reservation_phase_4c1_party_lineage`
- staging 最新：`20260827090512_reservation_phase_4c1_party_lineage`

Production 与独立 `badminton_stage` 均为 51 个版本。Stage 的 Phase 2 migration DDL/回填逻辑未变，只把四个冻结生产数据指纹替换为合成 fixture 的 staging 指纹；Phase 2 diagnostic 也必须使用相同 staging 指纹专门化，原样 production-fingerprint 版本会按设计 fail closed。Production migrations 45–51 均由 protected-branch integration 成功应用。Production 三个 manager read selectors 为 exact canonical，profile writer workflow fallback 仍为 legacy。
...
Confirmed Issue #157 adds append-only migration `20260826181644_reservation_phase_4b3_order_search`, replacing only the existing `admin_search_reservations(...)` definition while preserving its signature and version 1 envelope. The function now searches every Reservation Party, returns `party_count`, uses venue-local date bounds and `(matched_start_at, reservation_id)` keyset pagination, and enforces allowlisted Reservation/payment filters, a 367-day maximum range, and a 50-row page limit.
...
Phase 4B.1 is implemented, verified in isolated staging, and active for production manager schedule/capacity. `VITE_RESERVATION_SCHEDULE_READ_SOURCE` enables canonical rendering only for the exact value `canonical`; every missing or unknown value resolves to legacy. PR #151 injects the single repository variable into Pages with a pinned `legacy` fallback, and production currently sets it to exact canonical. `reservationScheduleRead.js` creates one explicit version 1 admin schedule view model that keeps the physical allocation ID, projection and effective Reservation/Session identities, membership/transition facts, Party contact, Court/time/amount/status/payment summaries, and separate legacy source traces. Missing required facts, invalid ranges, and duplicate allocation identities fail closed.

The canonical branch of `fetchAdminBookings` calls only `admin_list_reservation_allocations`, reusing venue-timezone boundaries and compound keyset pagination. AbortController plus a monotonic request ID blocks stale ranges. Schedule and capacity share the same mapped rows. Failure clears rows, keeps a bilingual error visible, and hides the grid; there is no per-request legacy fallback. Legacy shadow comparison runs only under the legacy source. Realtime remains `court_slots`-only, and those events now refresh whichever manager schedule/capacity view is open.

Staging API logs recorded ten successful canonical allocation RPC calls and zero direct `/rest/v1/bookings` schedule GETs during the run. Order search continues to use `admin_search_bookings` independently. Chinese/English desktop and 390×844 mobile checks rendered two Court allocations and three remaining Courts without overflow. The suite is 81 pass, one explicit no-local-PostgreSQL skip, and zero failures across 82 tests; lint and both production-legacy/canonical-staging builds pass. The hosted diagnostic remains 48 migrations, 192/192 memberships/allocations, 123 Reservations, zero mismatch, 17/0/17/3 writers, seven FORCE RLS tables, and `court_slots`-only Realtime. See [`docs/reservation-migration/phase-4b1-canonical-schedule.md`](./docs/reservation-migration/phase-4b1-canonical-schedule.md).

PR #149 merged at 2026-08-25 13:13:04 UTC as `560ecdc2d738b6bc42d03421e781ba73adac249c`. Pages run 32852015723 successfully built and deployed that commit. At that deployment, the live HTTP-200 page loaded `index-B_GCKKvu.js`, which contained zero canonical allocation RPC, staging project-ref, or schedule-source flag occurrences and one production project-ref occurrence. A real browser render completed without console errors or warnings. The post-deployment read-only diagnostic remained clean at 48 migrations, 192/192 memberships/allocations, 123 Reservations, zero mismatch, the 17/0/17/3 writer boundary, seven FORCE RLS tables, and `court_slots`-only Realtime. No database or canonical switch occurred in that default-legacy step; the later cutover is recorded below. See [`docs/reservation-migration/phase-4b1-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b1-default-legacy-production-verification.md).

The user later authorized the production canonical schedule/capacity cutover. PR #151 merged at 14:22:23 UTC as `6157f7a23daeb5d5c9c14d2d8310b41d24b7fd6d`, adding the deployment selector and legacy-fallback regression test. With the variable absent, Pages run 32859078691 first proved that the live artifact stayed `index-B_GCKKvu.js` with zero canonical RPCs. Exact canonical then produced `index-CvTu7GmI.js` in run 32859184175 with one canonical RPC occurrence, zero staging refs, and one production ref. Authenticated manager observation covered three schedule ranges and two capacity weeks with no read error and zero browser error/warn logs. API logs contained eight successful POST calls plus one successful OPTIONS request. The post-cutover diagnostic remained clean at 48 migrations, 192/192, 123, zero mismatch, 17/0/17/3, seven FORCE RLS tables, and `court_slots`-only Realtime. Rollback is setting/deleting the variable and rebuilding; no database rollback is required. Phase 4B.2 and decommission remain separate gates. See [`docs/reservation-migration/phase-4b1-production-cutover.md`](./docs/reservation-migration/phase-4b1-production-cutover.md).

Phase 4B.2 introduces an independent `VITE_RESERVATION_SELECTED_DETAIL_READ_SOURCE` selector. Only exact `canonical` enables it; all other values resolve to legacy. The supported staging pairing is canonical schedule plus canonical detail because the detail boundary requires the selected physical allocation ID and its effective Reservation/Session identities. Pages keeps a hard legacy fallback for this selector, so merging the code cannot cut over production detail by itself.

`reservationSelectedDetailRead.js` calls only `admin_get_reservation_detail(p_reservation_id)` and maps the Phase 4A DTO into an explicit version 1 `AdminReservationInspectorViewModel`. Its whitelist contains selection, Reservation, Party/role, Session/allocation, payment aggregate/share/safe-ledger, and lineage facts. It requires the selected allocation exactly once under the selected effective Session and reconciles Court, venue-local range, status, amount/currency, membership version, last transition, summary counts, the unique primary contact, payment references, and allowlisted payment enums. Any mismatch fails closed.

The loader runs only after selection and deduplicates/caches by effective Reservation in current-process memory. A new allocation in the same Reservation re-runs identity mapping without another RPC. A different Reservation aborts obsolete PostgREST work with `abortSignal`, while React request identity rejects late commits. Successful writers, undo/revert, detail updates, and `court_slots` Realtime invalidate cache. Errors expose only allowlisted codes, leave the schedule visible, and never silently fall back to legacy detail; cache never enters storage, console, or the database.

The card is explicitly read-only. Existing edit, payment, link, move, resize, swap, and cancellation controls retain their booking writer contracts. This phase changes no schema, migration, RLS, RPC grant, Realtime publication, Auth, or customer access. Full bilingual scope is in [`docs/reservation-migration/phase-4b2-canonical-selected-detail.md`](./docs/reservation-migration/phase-4b2-canonical-selected-detail.md).

PR [#154](https://github.com/tujiaqi2002/badminton/pull/154) merged at 2026-08-26 10:19:28 UTC as `92c892c28bf5458f62f84bc977007b872f3e1014`. Pages [run 32957663853](https://github.com/tujiaqi2002/badminton/actions/runs/32957663853) successfully deployed `index-wknub-uV.js`, containing one production ref and zero staging refs. `VITE_RESERVATION_SELECTED_DETAIL_READ_SOURCE` remains absent in Actions and resolves through the workflow's legacy fallback, while the schedule selector remains canonical. Selecting two live production allocations produced zero canonical-detail DOM panels, one legacy-notes panel per selection, successful canonical schedule calls, no `admin_get_reservation_detail` API log, and zero browser errors/warnings. Read-only diagnostics before and after deployment remained 48 migrations, 192/192, 123, zero mismatch, 17/0/17/3, seven FORCE RLS tables, and `court_slots`-only Realtime. Full evidence is in [`docs/reservation-migration/phase-4b2-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b2-default-legacy-production-verification.md).

PR #155 later merged that evidence as `a16e5f6`. The user separately authorized the production canonical detail gate. After a clean fresh diagnostic, the exact variable was set and [Pages run 32961018071](https://github.com/tujiaqi2002/badminton/actions/runs/32961018071) deployed `index-DLJKKuT_.js` from the same `main`, with one production ref, zero staging refs, and one detail RPC occurrence. The live manager rendered one ready canonical v1 card and zero error cards. Two sibling allocations in one three-Session / three-allocation Reservation retained the same reference and issued one POST total; crossing Reservations issued the next POST. Three Reservations produced three POST calls plus one OPTIONS request, all HTTP 200, with zero browser errors/warnings. A random authenticated non-manager was denied inside a read-only transaction and `anon` retained no EXECUTE. The post-cutover diagnostic was identical, with no migration, DB push, grant, writer, data, Auth, or Realtime change. Full evidence is in [`docs/reservation-migration/phase-4b2-production-cutover.md`](./docs/reservation-migration/phase-4b2-production-cutover.md).
...
Final merge-head CI [run 32837678041](https://github.com/tujiaqi2002/badminton/actions/runs/32837678041) passed 33/33 Reservation PostgreSQL tests and 14/14 adapter tests with zero skips under Node `v22.23.2`, pnpm `11.16.0`, and PostgreSQL 16, plus green lint/build. After explicit authorization, PR [#145](https://github.com/tujiaqi2002/badminton/pull/145) merged at 10:36:34 UTC as `ea3e6a80afcec64736a54b2bba65f0936f7e9ab8`. Post-merge Pages [run 32838082873](https://github.com/tujiaqi2002/badminton/actions/runs/32838082873) succeeded. The live page and CI share `index-ssrRc0gW.js`; the unset Actions variable falls back to false, and the live bundle contains neither the shadow event nor flag literal. Production/staging remain at 48 migrations.

Phase 4A.3 later completed a separately authorized bounded production observation. Default-off [run 32839332729](https://github.com/tujiaqi2002/badminton/actions/runs/32839332729) established the baseline. Feature-on [run 32839524612](https://github.com/tujiaqi2002/badminton/actions/runs/32839524612) deployed the same main commit and exercised four authenticated manager-schedule ranges. Browser logs produced four `info`-level clean events, while Supabase API logs showed four POST/HTTP 200 calls to each of `admin_list_reservation_allocations` and `admin_get_reservation_read_shadow_status`.

The post-observation diagnostic remained clean at 48 migrations, 192 bookings/memberships, 192 canonical allocations, 123 Reservations, the 17/0/17/3 writer boundary, seven FORCE RLS tables, and `public.court_slots`-only Realtime, with every Phase 3B/4A mismatch count at zero. The temporary variable was deleted and rollback [run 32839948133](https://github.com/tujiaqi2002/badminton/actions/runs/32839948133) restored the compiled-out `index-ssrRc0gW.js`. Legacy rows remained the only rendered source. Default schedule adoption, order/detail cutover, Stripe, and legacy decommission remain separate and unauthorized. Full bilingual evidence is in [`docs/reservation-migration/phase-4a3-production-shadow-observation.md`](./docs/reservation-migration/phase-4a3-production-shadow-observation.md).
