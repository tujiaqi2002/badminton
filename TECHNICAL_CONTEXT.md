...
### Phase 4B.1 canonical schedule/capacity source（Issue #148；staging 已验证，default-legacy 已部署）

`VITE_RESERVATION_SCHEDULE_READ_SOURCE` 是 build-time fail-closed source selector：只有 exact `canonical` 启用新路径，其他值均为 `legacy`。`.env.example` 明确 legacy，`.env.staging.example` 明确 canonical；生产 workflow 没有设置该变量。`reservationScheduleRead.js` 把 Phase 4A allocation DTO 映射为 explicit version 1 admin schedule view model，保留 physical allocation ID、projection/effective Reservation 与 Session、membership/transition、Party contact、Court/time/amount/status/payment summary，并把 legacy group/link 仅保留为 source trace。任何关键 identity/field 缺失、错误范围或重复 allocation 都 fail closed。
...
Phase 4B.1 is implemented, verified in isolated staging, and deployed to production in a default-legacy build. `VITE_RESERVATION_SCHEDULE_READ_SOURCE` enables canonical rendering only for the exact value `canonical`; every missing or unknown value resolves to legacy. The production example and workflow therefore remain legacy, while the staging example opts into canonical. `reservationScheduleRead.js` creates one explicit version 1 admin schedule view model that keeps the physical allocation ID, projection and effective Reservation/Session identities, membership/transition facts, Party contact, Court/time/amount/status/payment summaries, and separate legacy source traces. Missing required facts, invalid ranges, and duplicate allocation identities fail closed.
...
PR #149 merged at 2026-08-25 13:13:04 UTC as `560ecdc2d738b6bc42d03421e781ba73adac249c`. Pages run 32852015723 successfully built and deployed that commit. The live HTTP-200 page loads `index-B_GCKKvu.js`, which contains zero canonical allocation RPC, staging project-ref, or schedule-source flag occurrences and one production project-ref occurrence. A real browser render completed without console errors or warnings. The post-deployment read-only diagnostic remained clean at 48 migrations, 192/192 memberships/allocations, 123 Reservations, zero mismatch, the 17/0/17/3 writer boundary, seven FORCE RLS tables, and `court_slots`-only Realtime. No database or canonical production switch occurred. Phase 4B.2 detail/actions, production canonical cutover, and decommission remain separate gates. See [`docs/reservation-migration/phase-4b1-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b1-default-legacy-production-verification.md).
