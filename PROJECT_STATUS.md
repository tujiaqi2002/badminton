...
### Issue [#157](https://github.com/tujiaqi2002/badminton/issues/157) / Draft PR [#158](https://github.com/tujiaqi2002/badminton/pull/158)：Reservation Phase 4B.3 canonical order/search（staging 与 CI 已验证；等待 review）
...
- bundled Node `v24.19.0` / pnpm `11.19.0` 下 read suite 36/36；Reservation suite 37 total / 36 pass / 1 个 no-local-PostgreSQL skip / 0 fail；lint/build 通过。Draft PR #158 首轮 [CI run 33013724851](https://github.com/tujiaqi2002/badminton/actions/runs/33013724851) 在 Node `v22.23.2` / pnpm `11.16.0` / PostgreSQL `16.15` 下完成 Reservation 37/37、read 36/36、0 skip，并通过 lint/build；唯一 annotation 是 `pnpm/action-setup@v4` 的平台 Node 20 target 弃用提示。完整中英文设计与 staging 证据见 [`docs/reservation-migration/phase-4b3-canonical-order-search.md`](./docs/reservation-migration/phase-4b3-canonical-order-search.md)。下一门禁是 Draft PR review；merge 会触发 production migration 49，必须重新 preflight 并明确授权。
