...
- 多场地 booking group 默认继续整组移动和缩放。馆长选中其中一条后，可在详情中临时关闭“联动移动同组场地”；下一次拖动到空位或缩放只修改当前 booking，完成操作或切换选择后自动恢复整组联动。
- 临时单条移动不拆分订单，不清除 `booking_group_id` 或 `booking_link_id`。同组 booking 出现不同时段后，详情必须逐条显示真实场地和时间，不能继续假设整组同一时段。
