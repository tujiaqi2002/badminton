...
Phase 4B.2 已在 Issue #153 明确确认并进入 staging：馆长选中一片场地后可看到一张只读的统一 Reservation 卡，把主要联系人、其他参与人、整笔预约备注、当前 Session 备注、Session/allocation 范围、付款计划/已付/待付和 merge/split lineage 分开表达。同一 Reservation 的不同 Court allocation 共享该商业详情，但当前选中 allocation 与 Session identity 仍会严格核对。现有编辑、付款、关联、移动和取消按钮继续保持 legacy booking writer scope；界面明确说明这一兼容边界，不会暗示 action scope 已迁移。Production detail selector 仍为 legacy，数据库与客户登录均未改变。
...
Phase 4B.2 was explicitly approved in Issue #153 and is now in staging. Selecting one Court allocation gives the manager a read-only canonical Reservation card that separates the primary contact, other Parties, Reservation notes, selected-Session notes, Session/allocation scope, payment plan/paid/outstanding amounts, and merge/split lineage. Different Court allocations in one Reservation share the commercial detail, while the selected allocation and effective Session identities are still verified strictly. Existing edit, payment, relationship, move, and cancellation controls retain their legacy booking writer scopes, and the UI states that compatibility boundary instead of implying that action scopes have migrated. The production detail selector remains legacy; the database and customer login policy are unchanged.
