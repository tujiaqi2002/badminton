...
- 时间轴保持每 30 分钟 30px 的视觉尺度；单行高度随馆务 `slot_minutes` 等比例变化（15/30/60 分钟分别为 15/30/60px），让姓名、时间和必要状态在不同有效配置下都保持一致密度。
- “现在”时间线必须使用与排期行相同的时间尺度，不能因 `slot_minutes` 或行高变化偏离实际时间。
- 今天显示“现在”时间线。
...
- 默认使用最多 12 种可学习、可复用的低饱和纯色；按客户确定分配，同一客户在同一天保持一致，超过颜色词汇后有序复用，不继续制造一次性色相。
- 订单块贴合网格，不使用渐变或常态投影；颜色服务于快速定位，不把每笔订单做成悬浮卡片。
- 所有纯色保持统一感知明度和可读文字对比，避免黄色刺眼、深蓝过重。
- 可选“场地原色”模式只使用五种固定纯色，每片场地一种；不再按人数或客户继续改变同一场地的色相/浓淡。
- 付款状态使用独立语义层：未付、到店付款、处理中或失败等需要处理的状态显示小型高对比标签；已付与免费不在卡片上重复占位，但继续保留在详情和无障碍名称中。
- 默认卡片信息收敛为姓名与时间；备注、关联、多场地和循环只在存在时显示小型 icon/提示。
- 选中、Reservation 聚焦、关联、拖拽落点、过期或取消状态使用轮廓、灰度、形状或 icon；分组/关联和状态不能只靠颜色表达。
