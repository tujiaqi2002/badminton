...
Phase 4B.0 已建立只在 loopback、本地 staging flag/environment 与精确 project-ref 全部匹配时出现的 password 测试入口，并用 `badminton_stage` 的 disposable manager/non-manager 完成真实 Auth、馆长权限、桌面/手机排期与容量回归。它不开放普通客户登录，不改变生产 Auth 或生产页面，也没有让 canonical allocations 成为默认渲染来源；Phase 4B.1 schedule cutover 仍需单独确认。
...
Phase 4B.0 established a password test entry that appears only when the loopback host, local staging flag/environment, and exact expected project ref all match. Disposable manager and non-manager identities in `badminton_stage` completed real Auth, manager-permission, desktop/mobile schedule, and capacity regression. This opens no customer login, changes no production Auth or production page, and does not make canonical allocations the default render source. Phase 4B.1 schedule cutover remains a separately confirmed gate.
