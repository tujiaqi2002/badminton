...
Phase 4B.0 已建立只在 loopback、本地 staging flag/environment 与精确 project-ref 全部匹配时出现的 password 测试入口，并用 `badminton_stage` 的 disposable manager/non-manager 完成真实 Auth 与馆长权限回归。Phase 4B.1 让 canonical staging build 的馆长排期与容量监控直接读取 current Court allocations：同一 Reservation / Session 的多场地仍一片场地一张卡，但共享 effective identity；容量与排期共用同一数据。该能力已随 PR #149 部署生产，但生产缺省仍为 legacy，线上 artifact 不含 canonical schedule RPC，所以当前用户行为没有切换。canonical 模式读取失败会显示持续错误并隐藏排期/容量，不会把错误误报为全空，也不会静默切回旧数据。生产 canonical 默认切换、订单、选中详情和操作仍属于后续门禁。
...
Phase 4B.0 established a password test entry that appears only when the loopback host, local staging flag/environment, and exact expected project ref all match. Disposable manager and non-manager identities in `badminton_stage` completed real Auth and manager-permission regression. Phase 4B.1 lets a canonical staging build render manager schedule and capacity directly from current Court allocations. Multiple Courts in one effective Reservation/Session remain one card per physical allocation while sharing effective identities, and schedule/capacity consume the same rows. The capability is deployed from PR #149, but production still defaults to legacy and the live artifact contains no canonical schedule RPC, so current user behavior has not switched. A canonical read failure keeps a persistent error visible and hides schedule/capacity instead of claiming full availability or silently falling back. Production canonical default adoption, order, selected-detail, and action adoption remain later gates.
