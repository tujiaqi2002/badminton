...
Phase 4B.2 已由 PR #154 进入 `main` 并完成 default-legacy 生产发布验证：馆长选中一片场地后可在 exact-canonical staging build 看到一张只读的统一 Reservation 卡，把主要联系人、其他参与人、整笔预约备注、当前 Session 备注、Session/allocation 范围、付款计划/已付/待付和 merge/split lineage 分开表达。同一 Reservation 的不同 Court allocation 共享该商业详情，但当前选中 allocation 与 Session identity 仍会严格核对。生产 selected-detail selector 继续缺失并回退 legacy，所以线上仍显示原详情且不调用 canonical detail RPC。现有编辑、付款、关联、移动和取消按钮继续保持 legacy booking writer scope；数据库与客户登录均未改变。完整发布证据见 [`docs/reservation-migration/phase-4b2-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b2-default-legacy-production-verification.md)。
...
Phase 4B.2 entered `main` through PR #154 and completed a default-legacy production deployment verification. In an exact-canonical staging build, selecting one Court allocation gives the manager a read-only canonical Reservation card that separates the primary contact, other Parties, Reservation notes, selected-Session notes, Session/allocation scope, payment plan/paid/outstanding amounts, and merge/split lineage. Different Court allocations in one Reservation share the commercial detail while the selected allocation and effective Session identities remain strict. The production selected-detail selector is still absent and resolves to legacy, so the live inspector remains unchanged and sends no canonical detail RPC. Existing edit, payment, relationship, move, and cancellation controls retain their legacy booking writer scopes; the database and customer login policy are unchanged. Full evidence is in [`docs/reservation-migration/phase-4b2-default-legacy-production-verification.md`](./docs/reservation-migration/phase-4b2-default-legacy-production-verification.md).
