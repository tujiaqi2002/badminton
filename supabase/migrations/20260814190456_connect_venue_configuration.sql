-- Make venue_settings the authoritative source for every booking surface and
-- for the customer-facing booking/cancellation guards.
...
begin
  ...
  select * into v_settings from public.venue_settings settings where settings.singleton;
...
begin
  ...
  select jsonb_build_object(
    ...
    'settings', (select jsonb_build_object(
      'name_zh', settings.name_zh, 'name_en', settings.name_en,
...
begin
  ...
  select * into v_settings from public.venue_settings settings where settings.singleton;
  if v_booking.start_at <= timezone(v_settings.timezone, clock_timestamp())
      + make_interval(hours => v_settings.cancellation_notice_hours) then
    raise exception 'Booking cannot be cancelled within the configured cancellation window';
  end if;
...
comment on function public.get_venue_booking_configuration is 'Authenticated booking configuration for one venue-local date; venue_settings is the single source of truth.';
comment on function public.cancel_booking is 'Cancels a customer booking using the venue timezone and configured notice period.';
