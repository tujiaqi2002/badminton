...
begin
  ...
  select b.booking_group_id, c.sort_order into v_group_id, v_source_order
  from public.bookings b join public.courts c on c.id = b.court_id
  where b.id = p_booking_id and b.status in ('held', 'confirmed');
  ...
  select array_agg(b order by c.sort_order) into v_previous_rows
  from public.bookings b join public.courts c on c.id = b.court_id
  where b.booking_group_id = v_group_id and b.status in ('held', 'confirmed');
  if coalesce(array_length(v_previous_rows, 1), 0) = 0 then raise exception 'No active bookings found'; end if;

  -- Temporarily take the group out of the exclusion constraint inside this transaction.
  -- This lets adjacent groups shift by one court without colliding with themselves.
  ...
  foreach v_previous in array v_previous_rows loop
    ...
    from public.courts source
    join public.courts target on target.sort_order = source.sort_order + v_offset and target.status = 'open'
