...
export default function AdminRescheduleModal({ booking, busy, onClose, onSubmit, onMoved }) {
  ...
  const durations = useMemo(() => [...new Set([30, 60, 90, 120, currentDuration])].sort((a, b) => a - b), [currentDuration])
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form className="admin-reschedule-modal" onSubmit={submit}>
        ...
        <div className="admin-reschedule-fields">
          ...
          <label><span>{t('admin.reschedule.time')}</span><input required name="time" type="time" min="07:00" max="21:30" step="60" value={form.time} onChange={(event) => setForm((current) => ({ ...current, time: event.target.value }))} /></label>
