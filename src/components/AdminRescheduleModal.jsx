...
import { bookingDurations, COURTS, timeFromDateTime, venueNow } from '../lib/booking'
...
export default function AdminRescheduleModal({ booking, busy, onClose, onSubmit, onMoved, configuration }) {
  ...
  const durations = useMemo(() => [...new Set([...bookingDurations(configuration, true), currentDuration])].sort((a, b) => a - b), [configuration, currentDuration])
  const slotMinutes = Number(configuration?.settings?.slot_minutes || 30)
  const openMinute = Number(configuration?.opening_hours?.open_minute || 600)
  const closeMinute = Number(configuration?.opening_hours?.close_minute || 1440)
  const minuteToTime = (minute) => `${String(Math.floor(minute / 60)).padStart(2, '0')}:${String(minute % 60).padStart(2, '0')}`
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form className="admin-reschedule-modal" onSubmit={submit}>
        ...
        <div className="admin-reschedule-fields">
          ...
          <label><span>{t('admin.reschedule.time')}</span><input required name="time" type="time" min={minuteToTime(openMinute)} max={minuteToTime(closeMinute - slotMinutes)} step={slotMinutes * 60} value={form.time} onChange={(event) => setForm((current) => ({ ...current, time: event.target.value }))} /></label>
